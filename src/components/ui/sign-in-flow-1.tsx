...
const AnimatedNavLink = ({ href, children }: { href: string; children: React.ReactNode }) => {
  ...
  return (
    <RouterLink to={href} className={`group relative inline-block overflow-hidden h-5 flex items-center ${textSizeClass}`}>
    ...
    <header className={`fixed top-6 left-1/2 transform -translate-x-1/2 z-20
      ...
      <div className={`sm:hidden flex flex-col items-center w-full transition-all ease-in-out duration-300 overflow-hidden
        ...
        <nav className="flex flex-col items-center space-y-4 text-base w-full">
          {navLinksData.map((link) => (
            <RouterLink key={link.href} to={link.href} className="text-gray-300 hover:text-white transition-colors w-full text-center">
              {link.label}
            </RouterLink>
          ))}
        </nav>
...
export const SignInPage = ({ className, onEmailSubmit, onCodeSubmit, onSuccess }: SignInPageProps) => {
                ...
                {step === "email" ? (
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[1.8rem] text-white/70 font-light">登录您的账户</p>
                    </div>
                    ...
                    <p className="text-xs text-white/40 pt-10">
                      继续注册即表示您同意我们的{" "}
                      <RouterLink to="#" className="underline text-white/40 hover:text-white/60 transition-colors">
                        服务条款
                      </RouterLink>
                      、{" "}
                      <RouterLink to="#" className="underline text-white/40 hover:text-white/60 transition-colors">
